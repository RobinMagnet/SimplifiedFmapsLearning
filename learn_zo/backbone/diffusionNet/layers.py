...
class MiniMLP(nn.Sequential):
    ...
    def __init__(self, layer_sizes, dropout=False, dropout_prob=0.5, activation=nn.ReLU, name="miniMLP"):
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            if dropout and i > 0:
                self.add_module(name + "_mlp_layer_dropout_{:03d}".format(i), nn.Dropout(p=dropout_prob))
...
class DiffusionNetBlock(nn.Module):
    ...
    def __init__(self,
        ...
        # MLPs
        self.mlp = MiniMLP([self.MLP_C] + self.mlp_hidden_dims + [self.C_width], dropout=self.dropout, dropout_prob=self.dropout_prob)
