...
def run_trainer(trainer_cls):
    ...
    if cfg_cli.run_mode == 'train':
        ...
        run_cfg = OmegaConf.load(cfg_cli.run_cfg)
        template_cfg_path = OmegaConf.select(run_cfg, 'template_cfg')
        if template_cfg_path is not None:
            template_cfg = OmegaConf.load(template_cfg_path)
            run_cfg = OmegaConf.merge(template_cfg, run_cfg)
