...
def nn_dist_query(X,Y):
    formula = Genred('SqDist(X,Y)',
    ...
    return th.sqrt(formula(X,Y).squeeze(-1))
...
def nn_query_precise_torch(vert_emb, faces, points_emb, return_dist=False, batch_size=None, clear_cache=True):
    ...
    with th.no_grad():
        ...
        face_match, bary_coords = project_pc_to_triangles(vert_emb, faces, points_emb,
                                                         ...
                                                         verbose=False, leave_verbose=True)
...
def project_pc_to_triangles(vert_emb, faces, points_emb, precompute_dmin=True, batch_size=None, sparse_only=False, verbose=False, leave_verbose=True):
    """
    ...
    if precompute_dmin:
        Deltamin = nn_dist_query(vert_emb, points_emb)  # (n2,)
    ...
    if precompute_dmin or batch_size is None:
        faceind, bary = project_to_mesh_multi(vert_emb, faces, points_emb,
                                              th.arange(n_points), lmax, Deltamin, dmin=dmin, dmin_params=dmin_params)
        face_match = faceind
        bary_coord = bary
        ...
        for batchind in iterable:
            ...
            Deltamin_batch = nn_dist_query(vert_emb, points_emb[batch_minmax[0]:batch_minmax[1]].contiguous())
            ...
            faceind, bary = project_to_mesh_multi(vert_emb, faces, points_emb[batch_minmax[0]:batch_minmax[1]],
                                            th.arange(*batch_minmax) - batch_minmax[0], lmax, Deltamin_batch, # Deltamin[batch_minmax[0]:batch_minmax[1]],
...
def compute_per_triangle_min_dist(vert_emb, faces, points_emb, vert_sqnorm=None, points_sqnorm=None):
    ...
    th.minimum(distmat, mycdist(emb1, points_emb, sqnormX=vert_sqnorm[faces[:, 1]], sqnormY=points_sqnorm, squared=True), out=distmat)
    th.minimum(distmat, mycdist(emb2, points_emb, sqnormX=vert_sqnorm[faces[:, 2]], sqnormY=points_sqnorm, squared=True), out=distmat)
    th.sqrt(distmat, out=distmat)
...
def project_to_mesh_multi(vert_emb, faces, points_emb, vertinds, lmax, Deltamin, dmin=None, dmin_params=None):
    ...
    query_faceinds = th.where((deltamin - lmax[:,None] < Deltamin[None,vertinds]).any(1))[0]  # (p,) p < m1
...
class PointsTriangleProjLayer(nn.Module):
    ...
    def process_r4(self, final_s, final_t, final_dists, region_4, a, b, c, d, e, f, s, t, verbose=False):
        if verbose:
            print("R4:", region_4.sum())

        region_4_1 = region_4 & (d < 0)
        region_4_2 = region_4 & ~region_4_1
    ...
    def process_r3(self, final_s, final_t, final_dists, region_3, a, b, c, d, e, f, s, t, verbose=False):
        if verbose:
            print("R3:", region_3.sum())

        region_3_1 = region_3 & (e >= 0)
        region_3_2 = region_3 & ~region_3_1
    ...
    def process_r5(self, final_s, final_t, final_dists, region_5, a, b, c, d, e, f, s, t, verbose=False):
        if verbose:
            print("R5:", region_5.sum())
        region_5_1 = region_5 & (d >= 0)
        region_5_2 = region_5 & ~region_5_1
    ...
    def process_r0(self, final_s, final_t, final_dists, region_0, a, b, c, d, e, f, s, t, det, verbose=False):
        if verbose:
            print("R0:", region_0.sum())
        invDet = 1.0 / th.clamp(det[region_0],min=1e-6)
        # print('Det', det.min(), invDet.max())
        final_s[region_0] = s[region_0] * invDet
        final_t[region_0] = t[region_0] * invDet
        final_dists[region_0] = final_s[region_0] * (a[region_0] * final_s[region_0] + b[region_0] * final_t[region_0] + 2.0 * d[region_0]) +\
                                final_t[region_0] * (b[region_0] * final_s[region_0] + c[region_0] * final_t[region_0] + 2.0 * e[region_0]) + f[region_0]

        return final_s, final_t, final_dists

    def process_r2(self, final_s, final_t, final_dists, region_2, a, b, c, d, e, f, s, t, verbose=False):
        if verbose:
            print("R2:", region_2.sum())
        b_plus_d = b + d
        c_plus_e = c + e
        ...
        region_2_1 = region_2 & (c_plus_e > b_plus_d)
        region_2_2 = region_2 & ~region_2_1
    ...
    def process_r6(self, final_s, final_t, final_dists, region_6, a, b, c, d, e, f, s, t, verbose=False):
        if verbose:
            print("R6:", region_6.sum())
        tmp0 = b + e
        tmp1 = a + d

        region_6_1 = region_6 & (tmp1 > tmp0)
        region_6_2 = region_6 & ~region_6_1
    ...
    def process_r1(self, final_s, final_t, final_dists, region_1, a, b, c, d, e, f, s, t, verbose=False):
        if verbose:
            print("R1:", region_1.sum())
        numer = c + e - b - d
        denom = a - 2.0 * b + c

        region_1_1 = region_1 & (numer <= 0)
        region_1_2 = region_1 & ~region_1_1
    ...
    def return_all_projections(self, final_s, final_t, final_dists, bases, axis1, axis2, return_bary=True):
        # self.final_dists = th.clamp(self.final_dists, min=0.)
        final_dists[final_dists < 0] = 0  # (n,m)
        final_dists = th.sqrt(final_dists)
    ...
    def return_min_projections(self, final_s, final_t, final_dists, bases, axis1, axis2, return_bary=True):
        ...
        final_dists = th.sqrt(final_dists)  # (n,)

        final_s = th.gather(final_s, -1, argmin_proj).squeeze(dim=-1)  # (n,)
        final_t = th.gather(final_t, -1, argmin_proj).squeeze(dim=-1)  # (n,)

        # print("test nan", final_s.max(), final_t.max())

        projections = bases[argmin_proj.squeeze(-1)] + final_s[:,None] * axis1[argmin_proj.squeeze(-1)] + final_t[:,None] * axis2[argmin_proj.squeeze(-1)]  # (n,p)
        if return_bary:
            bary_coords = th.cat([
                1 - final_s[:, None] - final_t[:, None],
                final_s[:, None],
            ...
            dists, proj, bary, argmin_p = final_dists.detach().clone(), projections.detach().clone(), bary_coords.detach().clone(), argmin_proj.detach().squeeze(-1).clone()

            # del final_dists, projections, bary_coords, argmin_proj
            return dists, proj, bary, argmin_p
        
        return final_dists, projections, argmin_proj.squeeze(-1)

    def forward(self, triangles=None, points=None, min_only=True, return_bary=True, verbose=False):
        """
        triangles : (m, p , 3)
        points    : (n, p)
        """
        ...
        # print(a.max(), b.max(), c.max(), d.max(), e.max(), f.max(), det.max(), s.max(), t.max())

        region_list = self.get_base_regions(s, t, det)
        # print("test nan 0 ", final_s.max(), final_t.max())
        final_s, final_t, final_dists = self.process_r4(final_s, final_t, final_dists, region_list[4], a, b, c, d, e, f, s, t, verbose=verbose)
        # print("test nan 1", final_s.max(), final_t.max())
        final_s, final_t, final_dists = self.process_r3(final_s, final_t, final_dists, region_list[3], a, b, c, d, e, f, s, t, verbose=verbose)
        # print("test nan 2", final_s.max(), final_t.max())
        final_s, final_t, final_dists = self.process_r5(final_s, final_t, final_dists, region_list[5], a, b, c, d, e, f, s, t, verbose=verbose)
        # print("test nan 3", final_s.max(), final_t.max())
        final_s, final_t, final_dists = self.process_r0(final_s, final_t, final_dists, region_list[0], a, b, c, d, e, f, s, t, det, verbose=verbose)
        # print("test nan 4", final_s.max(), final_t.max())
        final_s, final_t, final_dists = self.process_r2(final_s, final_t, final_dists, region_list[2], a, b, c, d, e, f, s, t, verbose=verbose)
        # print("test nan 5", final_s.max(), final_t.max())
        final_s, final_t, final_dists = self.process_r6(final_s, final_t, final_dists, region_list[6], a, b, c, d, e, f, s, t, verbose=verbose)
        # print("test nan 6", final_s.max(), final_t.max())
        final_s, final_t, final_dists = self.process_r1(final_s, final_t, final_dists, region_list[1], a, b, c, d, e, f, s, t, verbose=verbose)
        # print("test nan 7", final_s.max(), final_t.max())

        if min_only:
            res = self.return_min_projections(final_s, final_t, final_dists, bases, axis1, axis2, return_bary=return_bary)
            return res
        
        return self.return_all_projections(final_s, final_t, final_dists, bases, axis1, axis2, return_bary=return_bary)

class PointsTriangleProjLayer2(nn.Module):
    def __init__(self):
        # pass
        super().__init__()
